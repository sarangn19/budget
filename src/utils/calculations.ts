...
import { getStartOfMonth, getEndOfMonth, isDateInRange } from './helpers';
...
export const calculateSafeToSpend = (
  ...
  const plannedExpenses = budget
    ? (budget.essentialExpenses || 0) + (budget.variableExpenses || 0) + (budget.personalSpending || 0)
    : 0;

  return Math.max(0, totalBalance - upcomingCommitments - safetyBuffer - plannedExpenses * 0.3);
};
